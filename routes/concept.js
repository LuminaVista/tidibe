...
concept.put('/ai/answer/edit/:concept_answer_id', authenticate, async (req, res) => {

    let connection;

    try {
        let { concept_answer_id } = req.params;
        concept_answer_id = parseInt(concept_answer_id, 10);
        ...
        await connection.execute(
            `UPDATE Concept_Answers 
             SET answer = ?, answer_status = 'approved'
             WHERE concept_answer_id = ?;`,
            [answer_content, concept_answer_id]
        );

        return res.status(200).json({
            concept_answer_id: concept_answer_id,
